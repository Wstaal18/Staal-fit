    ...
    <SectionWrapper id={'generate'} header={"generate your workout"} title={['It\'s', ' Huge', ' o\'clock']}>
          ...
          {Object.keys(WORKOUTS).map((type, typeIndex) => {
            return(
              <button onClick={() => {
                  setMuscles([])
                  setPoison(type)
              }} className={'bg-slate-950 border duration-200 px-4 hover:border-blue-600 py-3 rounded-lg' + (type === poison ? ' border-blue-600 rounded-lg' : 'border-blue-400 rounded-lg')} key={typeIndex}>
       ...
       {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
        return(
          ...
          <button onClick={() => {
            setGoal(scheme)
        }} className={'bg-slate-950 border duration-200 hover:border-blue-600 py-3 rounded-lg px-4' + (scheme === goal ? ' border-blue-600 rounded-lg' : 'border-blue-400 rounded-lg')} key={schemeIndex}>
